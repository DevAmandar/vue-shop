<template>
  <div>
    <header
      class="flex justify-center items-center px-2 relative -z-2"
      :class="{ 'bg-neutral-950': isDarkPinia, 'bg-white': !isDarkPinia }"
    >
      <!-- circles -->
      <div
        class="hidden lg:block absolute right-[10%] top-11 -z-1 w-[220px] h-[220px] rounded-[100%] bg-gradient-to-t from-purple-700 to-purple-400"
      ></div>
      <div
        class="hidden lg:block absolute left-[20%] bottom-11 -z-1 w-[160px] h-[160px] rounded-[100%] bg-gradient-to-t from-purple-700 to-purple-400"
      ></div>
      <!-- /circles -->
      <div class="mt-28 text-center">
        <h1
          class="md:text-5xl sm:text-4xl text-2xl poppinsM font-bold tracking-tight"
          :class="{ 'text-white': isDarkPinia }"
        >
          Make your Outfit
          <span
            class="bg-gradient-to-r from-purple-700 via-purple-500 bg-clip-text text-transparent"
            :class="{ 'to-black/50': isDarkPinia, 'to-white/50': !isDarkPinia }"
            >wonderful</span
          >
        </h1>
        <p
          class="max-w-[800px] mt-7 font-semibold text-[17px] text-center"
          :class="{ 'text-slate-300': isDarkPinia, 'text-zinc-800': !isDarkPinia }"
        >
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Facere soluta iusto expedita
          veniam asperiores, cumque id, tempora numquam voluptatibus.
        </p>
        <div class="mt-7 mb-24 flex justify-center items-center gap-3">
          <button class="px-7 py-3 rounded-sm bg-violet-700 text-white font-bold cursor-pointer">
            Start Shopping
          </button>
          <button class="px-7 py-3 rounded-sm bg-neutral-700 text-white font-bold cursor-pointer">
            Learn More
          </button>
        </div>
      </div>
    ...
    <main class="mt-16 mb-16">
      ...
      <div class="relative">
        ...
        <div
          class="text-center"
          :class="{ 'bg-slate-100': !isDarkPinia, 'bg-zinc-900': isDarkPinia }"
        >
          <h4 class="text-sm" :class="{ 'text-black': !isDarkPinia, 'text-white': isDarkPinia }">
            Explore the solutions
          </h4>
          <p
            class="max-w-[800px] font-medium text-[17px] text-center mx-auto mt-4"
            :class="{ 'text-slate-300': isDarkPinia, 'text-zinc-800': !isDarkPinia }"
          >
        ...
        <!-- concept main -->
        <div
          class="concept-main flex justify-center items-center not-md:flex-col not-md:gap-32 gap-2 md:px-16"
          :class="{ 'bg-slate-100': !isDarkPinia, 'bg-zinc-900': isDarkPinia }"
        >
          <!-- lef concept -->
          <div class="mt-12 text-left flex-4 z-2">
            <div class="">
              <!-- title -->
              <h4
                class="text-sm font-semibold not-md:text-center"
                :class="{ 'text-black': isDarkPinia, 'text-white': isDarkPinia }"
              >
                Powerful suite of tools
              </h4>
              <p
                class="max-w-[500px] font-medium text-[17px] mt-4 not-md:mx-auto"
                :class="{ 'text-slate-300': isDarkPinia, 'text-zinc-800': !isDarkPinia }"
              >
              ...
              <div class="card flex flex-col gap-0.5">
                <div
                  ...
                  class="not-md:mx-auto transition-all duration-300 ease-in-out flex justify-center items-center gap-1 mt-5 max-w-[500px] px-5 py-3.5 border-[5px] rounded-md shadow-md text-left cursor-pointer"
                  :class="{
                    'bg-neutral-800 border-neutral-700': isDarkPinia && !card.isActiveCard,
                    'bg-slate-100 border-white': !isDarkPinia && !card.isActiveCard,
                    'bg-neutral-900 border-violet-700': card.isActiveCard && isDarkPinia,
                    'bg-neutral-800 border-violet-700': card.isActiveCard && !isDarkPinia,
                  }"
                ...
                >
                  <div>
                    <h5
                      ...
                      :class="{
                        'text-black': !isDarkPinia && !card.isActiveCard,
                        'text-teal-50': !isDarkPinia && card.isActiveCard,
                        'text-white': isDarkPinia,
                      }"
                    >
                      {{ card.title }}
                    </h5>
                    <p
                      class="max-w-[500px] font-medium text-[17px] mt-2"
                      :class="{
                        'text-zinc-800': !isDarkPinia && !card.isActiveCard,
                        'text-slate-100': isDarkPinia && !card.isActiveCard,
                        'text-slate-200': !isDarkPinia && card.isActiveCard,
                        'text-slate-300': isDarkPinia && card.isActiveCard,
                      }"
                    >
                      {{ card.description }}
                    </p>
                  ...
                  <component
                    :is="card.icon"
                    class="transition-all duration-300 ease-in-out size-10 not-md:size-8 shrink-0 p-1 rounded-[100%] shadow-xl"
                    :class="{
                      'text-white': isDarkPinia && !card.isActiveCard,
                      'text-black bg-white': !isDarkPinia && !card.isActiveCard,
                      'text-white bg-neutral-700': card.isActiveCard && isDarkPinia,
                      'text-black bg-white': card.isActiveCard && !isDarkPinia,
                    }"
                  />
                </div>
              </div>
          ...
          <!-- right concept -->
          <div class="flex-3 z-2">
            <Transition mode="out-in" name="fade">
              <img :key="activeCardId" :src="currentImage" class="object-cover" alt="" />
            </Transition>
          </div>
        </div>
        <!-- absolot bg black -->
        <div
          class="w-full h-[150px] absolute not-md:bottom-[-80px] bottom-[-100px] z-1"
          :class="{ 'bg-white': !isDarkPinia, 'bg-zinc-950': isDarkPinia }"
        ></div>
      </div>
      <!-- section 2 -->
      <div class="section2 relative">
        <!-- title -->
        <div
          class="text-center md:mt-[99px] mt-[79px] pb-6 relative"
          :class="{ 'bg-white': !isDarkPinia, 'bg-zinc-950': isDarkPinia }"
        >
          <h4 class="text-sm" :class="{ 'text-black': !isDarkPinia, 'text-white': isDarkPinia }">
            How Simple works
          ...
          <!-- absolot bg black -->
          <div
            class="w-full h-[260px] absolute bottom-[-260px] z-3"
            :class="{ 'bg-white': !isDarkPinia, 'bg-neutral-950': isDarkPinia }"
          ></div>
        </div>
        <!-- services -->
        <div
          class="flex justify-center items-center mt-16 px-16 py-12 flex-wrap gap-2.5 bg-zinc-900"
        >
          <div
            class="text-center flex justify-center items-center flex-col shadow-xl/30 py-8 px-8 rounded-md z-4"
            :class="{ 'bg-neutral-900': isDarkPinia, 'bg-slate-100': !isDarkPinia }"
            v-for="service in services"
          >
            <component :is="service.icon" class="size-12 text-violet-700" />
            <h5
              class="poppinsM font-semibold mt-3"
              :class="{ 'text-white': isDarkPinia, 'text-black': !isDarkPinia }"
            >
              {{ service.title }}
            </h5>
            <p
              class="poppinsL max-w-[240px] mt-2"
              :class="{ 'text-slate-300': isDarkPinia, 'text-zinc-800': !isDarkPinia }"
            >
              {{ service.description }}
            </p>
      ...
      <div>
        <!-- title -->
        <div
          class="text-center pt-[79px] pb-6"
          :class="{ 'bg-white': !isDarkPinia, 'bg-zinc-950': isDarkPinia }"
        >
          <h4 class="text-sm" :class="{ 'text-black': !isDarkPinia, 'text-white': isDarkPinia }">
          ...
          <p
            ...
            :class="{ 'text-slate-300': isDarkPinia, 'text-zinc-800': !isDarkPinia }"
          >
            Arcu cursus vitae congue mauris rhoncus viverra nibh cras pulvinar mattis blandit libero
            cursus mattis.
          </p>
        </div>
        <!-- icons -->
        <div
          class="flex justify-center items-center gap-14 flex-wrap py-12"
          :class="{ 'bg-zinc-950': isDarkPinia, 'bg-white': !isDarkPinia }"
        >
          <div
        ...
        <!-- comment -->
        <div class="py-14" :class="{ 'bg-zinc-950': isDarkPinia, 'bg-white': !isDarkPinia }">
          <div
            class="mx-10 text-left not-lg:text-center px-10 py-16 bg-zinc-800 rounded-sm shadow-violet-300 relative overflow-hidden"
            :class="{ 'shadow-xl/5': isDarkPinia, 'shadow-lg': !isDarkPinia }"
          >
            <!-- circle -->
            <div
              class="hidden lg:block absolute -right-[90px] -top-[3px] size-96 rounded-full bg-radial-[at_25%_25%] from-violet-200 to-violet-700 to-35%"
            ></div>
            <div
              class="hidden lg:block absolute right-[280px] top-[10px] size-12 rounded-full bg-violet-400 opacity-30"
            ></div>
            <div
              class="hidden lg:block absolute right-[320px] top-[80px] size-1 rounded-full bg-violet-400 opacity-30"
            ></div>
            <div
              class="hidden lg:block absolute right-[330px] bottom-[40px] size-16 rounded-full bg-violet-400 opacity-30"
            ></div>
            <div
              class="hidden lg:block absolute right-[350px] bottom-[60px] size-6 rounded-full bg-violet-400 opacity-50"
            ></div>
            <!-- /circle -->
            <h5 class="text-sm font-semibold text-white">Powering your business</h5>
            <p class="max-w-[500px] font-medium text-[17px] text-slate-300 mt-4 not-lg:mx-auto">
              Lorem ipsum dolor sit amet consectetur adipisicing elit nemo expedita voluptas culpa
              sapiente.
            </p>
            <div
              class="flex not-md:flex-col mt-5 not-lg:justify-center justify-start items-center gap-2.5"
            >
              <input
                class="border border-violet-400 h-10 outline-0 rounded-sm px-3 text-white"
                type="email"
                placeholder="Your email..."
              />
              <button
                class="bg-violet-700 rounded-md text-white font-semibold px-10 py-2.5 text-center cursor-pointer"
              >
                Subscribe
              </button>
            </div>
...
const imgs = ref([
  { src: '/src/assets/image/illustration2.svg', id: 1 },
  { src: '/src/assets/image/illustration3.svg', id: 2 },
  { src: '/src/assets/image/illustration4.svg', id: 3 },
])
// services
const services = ref([
  {
...
function handleCardClick(id) {
  cards.value.forEach((el) => {
    if (el.id == id) {
      el.isActiveCard = true
      activeCardId.value = id
    } else {
      el.isActiveCard = false
    }
  })
  imgs.value.forEach((im) => {
    if (im.id == id) {
      currentImage.value = im.src
    }
  })
}
</script>
